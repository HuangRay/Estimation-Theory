...
fMatrix Cofactor(const fMatrix &A)
{
	...
	for (int i = 0; i < A.rows*A.cols; i++) {
		...
		for(int j = 0; j<cof.rows*cof.cols; j++){
			...
			cof.elem[j] = A.elem[num_elem];
			num_elem++;
		}
		//B.elem[i] = pow((-1), (i%A.rows + i / A.cols))*Determinant(cof[i]);
		B.elem[i] = pow((-1), (i%A.rows + i / A.cols))*Determinant(cof);
		//B.elem[i] = pow((-1), i)*Determinant(cof[i]);	
		//cout  <<  "\t" <<  pow((-1), (i)) << "\t" << pow((-1), (i+i/A.cols))/* << "\t" <<B.elem[i]*/ << endl;
		//cout << (i%A.cols) << "\t" << pow((-1), (i))*Determinant(cof[i]) << "\t" << pow((-1), (i + i / A.cols))*Determinant(cof[i]) << endl;
	}
	//B.Show();
	//cout << endl;
	//delete[] cof;
...
fMatrix  Inverse(const fMatrix &A)// Computes the inverse of a square matrix.
{
	...
	*/
	fMatrix B(A);
	//Cofactor(A).Show();
	//std::cout << Determinant(A) << endl;
	//std::cout << Determinant(B) << endl;
	B = (1 / Determinant(B)) * Transp(Cofactor(B));
	return B;
}
